    ...
    public String listEsBlogs(
        ...
        try {
            if (order.equals("hot")) { // 最热查询 根据阅读量、评论量、点赞量、时间来逆序
                ...
                page = blogService.listHotestBlogs(keyword, pageable);
            } else if (order.equals("new")) { // 最新查询 根据时间逆序
                Sort sort = new Sort(Direction.DESC,"createTime"); 
                Pageable pageable = PageRequest.of(pageIndex, pageSize, sort);
                page = blogService.listNewestBlogs(keyword, pageable);
            }
        ...
        if (!async && !isEmpty) {
            ...
            List<User> users = blogService.listTop5Users();
            // 管理员账户和游客账户不在前台页面显示
...
//            	if(it.hasNext()){
//            		User user = it.next();
//            		if("admin".equals(user.getUsername()) || "visitor".equals(user.getUsername())){
//            			it.remove();
//            		}
//            	}
            }
